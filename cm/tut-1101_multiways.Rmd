...
We replace a data frame in a ggplot graph we've already defined using the `%+%` operator---similar to using `+` to add a new layer. I replace the  *aes()* and *facet_wrap()* using `+` as usual.  
...
If we want to assess how the different ethnicity/sex groups are distributed across a major, we have to switch the roles of rows and panels. To do that, we swap the *y* variable with the *facet_wrap* variable, creating the *dual* of the original graph.  When plotting multiway data, I recommend you always plot both versions. One may tell a more compelling story than the other. 
...
To practice importing an image from the *visuals* directory to an Rmd file, open  *tut04_second-report.Rmd* from your *practiceR* directory. 
